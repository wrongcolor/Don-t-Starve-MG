import { useState } from 'react'
import { useModProjectStore } from '../../store/modProjectStore'
import { CharacterForm } from '../forms/CharacterForm'
import { btnPrimary, btnSecondary, btnDanger } from '../forms/FormField'
import type { CharacterDef } from '../../types/modProject'

export function CharactersPanel() {
  const characters = useModProjectStore((s) => s.project.characters)
  const upsertCharacter = useModProjectStore((s) => s.upsertCharacter)
  const removeCharacter = useModProjectStore((s) => s.removeCharacter)

  const [editingId, setEditingId] = useState<string | null>(null)
  const [adding, setAdding] = useState(false)

  const editingCharacter = editingId ? characters.find((c) => c.id === editingId) : undefined

  const handleSave = (character: CharacterDef) => {
    upsertCharacter(character)
    setEditingId(null)
    setAdding(false)
  }

  return (
    <div className="space-y-4 max-w-2xl">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Personagens jogáveis</h2>
        {!adding && !editingId && (
          <button className={btnPrimary} onClick={() => setAdding(true)}>
            + Novo personagem
          </button>
        )}
      </div>

      {characters.length === 0 && !adding && (
        <p className="text-sm text-slate-500 dark:text-slate-400">Nenhum personagem adicionado ainda.</p>
      )}

      {!adding && !editingId && (
        <ul className="space-y-1">
          {characters.map((character) => (
            <li
              key={character.id}
              className="flex items-center justify-between rounded-md border border-slate-200 dark:border-slate-700 px-3 py-2"
            >
              <div>
                <span className="text-sm font-medium text-slate-900 dark:text-slate-100">{character.name}</span>
                <span className="ml-2 text-xs text-slate-400">#{character.id}</span>
              </div>
              <div className="flex gap-2">
                <button className={btnSecondary} onClick={() => setEditingId(character.id)}>
                  Editar
                </button>
                <button className={btnDanger} onClick={() => removeCharacter(character.id)}>
                  Remover
                </button>
              </div>
            </li>
          ))}
        </ul>
      )}

      {(adding || editingCharacter) && (
        <CharacterForm
          key={editingId ?? 'new'}
          initialCharacter={editingCharacter}
          onSave={handleSave}
          onCancel={() => {
            setAdding(false)
            setEditingId(null)
          }}
        />
      )}
    </div>
  )
}
