...
describe('generateStaticLayoutLua/generateStaticLayoutRegistration', () => {
  ...
  it('generates a registration snippet using the confirmed real API (patterns.md#22, #55)', () => {
    ...
    expect(code).not.toContain('LAYOUT.STATIC')
    expect(code).toContain('layout_position = GLOBAL.LAYOUT_POSITION.CENTER')
    expect(code).toContain('start_mask = GLOBAL.PLACE_MASK.IGNORE_IMPASSABLE_BARREN_RESERVED')
    expect(code).toContain('fill_mask = GLOBAL.PLACE_MASK.IGNORE_IMPASSABLE_BARREN_RESERVED')
  })
})
