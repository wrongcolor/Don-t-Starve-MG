...
function lootBlock(creature: CreatureDef): string[] {
  ...
  for (const drop of creature.loot) {
    lines.push(`    inst.components.lootdropper:AddChancedLoot("${drop.prefab}", ${drop.chance})`)
  }
  return lines
}

// PLACEHOLDER assets: reuses no vanilla fallback here since creatures have no safe
// "borrow Wilson's build" equivalent — the user must supply anim/<id>.zip themselves
...
export function generateCreaturePrefab(creature: CreatureDef): string {
  ...
  lines.push('')
  lines.push(`    inst.AnimState:SetBank("${creature.id}")`)
  lines.push(`    inst.AnimState:SetBuild("${creature.id}")`)
  ...
  for (const tag of creature.tags) lines.push(`    inst:AddTag("${tag}")`)
  lines.push('')
