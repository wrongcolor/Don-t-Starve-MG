...
- **Itens**: `images/inventoryimages/<id>.xml`/`.tex` (ícone de inventário, sempre necessário).
  - `huntersknife`: precisa de `anim/huntersknife.zip` (build/bank "huntersknife", animação "idle").
    - Por ser empunhável, também precisa de `anim/swap_huntersknife.zip` (aparência na mão do personagem).
  - `huntersspade`: precisa de `anim/huntersspade.zip` (build/bank "huntersspade", animação "idle").
    - Por ser empunhável, também precisa de `anim/swap_huntersspade.zip` (aparência na mão do personagem).
  - `travelpack`: precisa de `anim/travelpack.zip` (build/bank "travelpack", animação "idle").
  - `toolbelt`: precisa de `anim/toolbelt.zip` (build/bank "toolbelt", animação "idle").
    - Também precisa de `anim/ui_toolbelt.zip` (arte de UI do contêiner, 6 slots).
