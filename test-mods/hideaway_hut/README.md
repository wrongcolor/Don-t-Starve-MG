...
- **Estruturas**: `images/inventoryimages/<id>.xml`/`.tex` (ícone de inventário, sempre necessário).
  - `hideawayhut`: reaproveita o build "pig_house" do jogo base — nenhum `anim/*.zip` próprio é necessário.
...
Este mod usa `StructureDef.interior` (patterns.md#64), que depende do
componente `interiorspawner` do mod publicado **"Above the Clouds"**
(Workshop id `workshop-3322803908`, já declarado em `mod_dependencies` no
`modinfo.lua` gerado). Sem ele ativo, as estruturas com interior não vão
conseguir criar a sala/porta ao serem construídas.

## Como instalar

1. Inscreva-se e ative **"Above the Clouds"** primeiro (ver "Dependência obrigatória" acima).
