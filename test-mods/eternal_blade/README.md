...
- **Itens**: `images/inventoryimages/<id>.xml`/`.tex` (ícone de inventário, sempre necessário).
  - `eternalblade`: precisa de `anim/eternalblade.zip` (build/bank "eternalblade", animação "idle").
    - Por ser empunhável, também precisa de `anim/swap_eternalblade.zip` (aparência na mão do personagem).
