...
- **Criaturas**: por padrão reaproveitam o build "pigman" do jogo base — só precisam de `anim/<id>.zip` próprio se a animação for explicitamente marcada como personalizada.
  - `meadowhopper`: reaproveita o build "pigman" do jogo base — confirme em-jogo que as animações "idle"/"walk"/"atk"/"hit"/"death" existem nesse build antes de publicar (não verificado por esta ferramenta).
