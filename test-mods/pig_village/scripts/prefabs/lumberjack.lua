...
local function fn()
    ...
    inst.components.locomotor.walkspeed = TUNING.LUMBERJACK_WALKSPEED

    inst:AddComponent("health")
    inst.components.health:SetMaxHealth(TUNING.LUMBERJACK_HEALTH)

    inst:AddComponent("combat")
    inst.components.combat:SetDefaultDamage(TUNING.LUMBERJACK_DAMAGE)
    inst.components.combat:SetAttackPeriod(TUNING.LUMBERJACK_ATTACK_PERIOD)
    ...
    inst:SetStateGraph("SGlumberjack")
    inst:SetBrain(require("brains/lumberjackbrain"))

    return inst
end

return Prefab("lumberjack", fn, assets, prefabs)
